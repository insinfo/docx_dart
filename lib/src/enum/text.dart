...
enum WD_PARAGRAPH_ALIGNMENT {
  ...
  JUSTIFY_LOW(
      8, "lowKashida", "Justified with a low character compression ratio."),
  ...
  static WD_PARAGRAPH_ALIGNMENT fromXml(String? xmlValue) {
    if (xmlValue == null) {
      throw ArgumentError(
          'WD_PARAGRAPH_ALIGNMENT.fromXml() requires a non-null String argument');
    }
    ...
    throw ArgumentError(
        'WD_PARAGRAPH_ALIGNMENT has no XML mapping for "$xmlValue"');
  }
...
enum WD_BREAK_TYPE {
  ...
  LINE_CLEAR_ALL(
      11, 'Line break clearing all (added for consistency, not in MS version)'),
  PAGE(7, 'Page break'),
  ...
  TEXT_WRAPPING(
      11, 'Text wrapping break'); // Note: Value is same as LINE_CLEAR_ALL
...
enum WD_LINE_SPACING {
  ...
  static WD_LINE_SPACING fromXml(String? xmlValue) {
    if (xmlValue == null) {
      throw ArgumentError(
          'WD_LINE_SPACING.fromXml() requires a non-null String argument');
    }
    for (final member in values) {
...
enum WD_TAB_ALIGNMENT {
  ...
  static WD_TAB_ALIGNMENT fromXml(String? xmlValue) {
    if (xmlValue == null) {
      throw ArgumentError(
          'WD_TAB_ALIGNMENT.fromXml() requires a non-null String argument');
    }
    for (final member in values) {
      if (member.xmlValue == xmlValue) {
        return member;
...
enum WD_TAB_LEADER {
  ...
  static WD_TAB_LEADER fromXml(String? xmlValue) {
    if (xmlValue == null) {
      throw ArgumentError(
          'WD_TAB_LEADER.fromXml() requires a non-null String argument');
    }
    for (final member in values) {
      if (member.xmlValue == xmlValue) {
        return member;
...
enum WD_UNDERLINE {
  ...
  DOT_DASH_HEAVY(
      25, "dashDotHeavy", "Alternating heavy dots and heavy dashes."),
