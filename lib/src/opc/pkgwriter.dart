...
class PackageWriter {
  static void write(
      dynamic pkgFile, Relationships pkgRels, Iterable<Part> parts) {
  ...
  static void _writeContentTypesStream(
      PhysPkgWriter physWriter, Iterable<Part> parts) {
  ...
  static void _writeParts(PhysPkgWriter physWriter, Iterable<Part> parts) {
    for (final part in parts) {
      ...
      if (part.rels.isNotEmpty) {
        physWriter.write(
            part.partname.relsUri,
            Uint8List.fromList(part
                .rels.xml.codeUnits)); // Assumindo que rels.xml retorna String
      }
    }
  }
  ...
  static void _writePkgRels(PhysPkgWriter physWriter, Relationships pkgRels) {
    physWriter.write(
        PACKAGE_URI.relsUri,
        Uint8List.fromList(
            pkgRels.xml.codeUnits)); // Assumindo que rels.xml retorna String
  }
}
...
class ContentTypesItem {
  ...
  factory ContentTypesItem.fromParts(Iterable<Part> parts) {
    ...
    cti._defaults['rels'] =
        CONTENT_TYPE.OPC_RELATIONSHIPS; // Exemplo, buscar de constantes
  ...
  void _addContentType(PackUri partname, String contentType) {
    ...
    final isDefault = defaultContentTypes
        .any((pair) => pair.$1 == ext && pair.$2 == contentType);
  ...
  opc_oxml.CT_Types get _element {
    final typesElm =
        opc_oxml.CT_Types.newTypes(); // Supondo um construtor estático
    final sortedDefaults = _defaults.entries.toList()
      ..sort((a, b) => a.key.compareTo(b.key));
    final sortedOverrides = _overrides.entries.toList()
      ..sort((a, b) => a.key.compareTo(b.key));
